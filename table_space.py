...
def squared_margin_error(x: Tensor, target_rows: Tensor, target_cols: Tensor) -> Tensor:
...
def exact_margins_satisfied(x: Tensor, target_rows: Tensor, target_cols: Tensor) -> Tensor:
    rs = row_sums(x)
    cs = col_sums(x)
...
def sample_uniform_tables(num_samples: int, m: int, n: int, total_count: int, generator: torch.Generator=None, device: str='cpu') -> Tensor:
    d = m * n
    ...
    for i in iterator:
        ...
        counts = [boundaries[k + 1] - boundaries[k] - 1 for k in range(len(boundaries) - 1)]
